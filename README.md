...
install directory and look for `vcvars64.bat`. Copy the path to this script
    > Example: C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvars64.bat
...
5. Execute below commands to get the necessary compile executables for applicable use-cases
    ...
    **Debug version**
    ```shell
    qmake qtac-workspace.pro -spec win32-msvc "CONFIG+=debug" "CONFIG+=qml_debug" && %QTJOM% qmake_all
    ```

    **Release version**
    ```shell
    qmake.exe qtac-workspace.pro -spec win32-msvc "CONFIG+=qtquickcompiler" && %QTJOM% qmake_all
    ```
...
4. Execute below commands to get the necessary compile executables for applicable use-cases

    **Debug version**
    ```shell
    $QTBIN/qmake qtac-workspace.pro -spec linux-g++ CONFIG+=debug CONFIG+=qml_debug && /usr/bin/make qmake_all
    ```

    **Release version**
    ```shell
     $QTBIN/qmake qtac-workspace.pro -spec linux-g++ CONFIG+=qtquickcompiler && /usr/bin/make qmake_all
    ```
...
- `QTJOM`: `C:\Qt\Tools\QtCreator\bin\jom\jom.exe`
...
export QTJOM=/path/to/Qt/directory/Tools/QtCreator/bin/jom/jom
```
