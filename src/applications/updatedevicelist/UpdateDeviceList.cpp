...
void UpdateDeviceList::save(const QString &filePath)
{
	...
	for (const auto& entryInfo: entryInfoList)
	{
		...
		if (platformConfiguration.isNull() == false)
		{
			...
			// may run in an environment whose config root (tacConfigRoot()) has
			// nothing to do with where an end-user's installed package will look
			// for its configuration files (e.g. a CI build runner's checkout path).
			// Writing a bare filename here defers path resolution to runtime, where
			// PlatformConfiguration::getConfiguration() resolves it against the
			// correct tacConfigRoot() on whatever machine actually runs the app.
